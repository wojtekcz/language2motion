...
// func update(model: inout TransformerModel, using optimizer: inout Adam<TransformerModel>, for batch: MotionLangBatch) -> Float {
//     // print("update() - start")
...
// func validate(model: inout TransformerModel, for batch: MotionLangBatch) -> Float {
//     let labels = batch.targetTruth.reshaped(to: [-1])
//     let resultSize = batch.targetTruth.shape.last! * batch.targetTruth.shape.first!
//     let padIndex = textProcessor.padId
...
// let exampleIndex = 1
// var source = MotionLangBatch(tokenIds: batch2.tokenIds[exampleIndex].expandingShape(at: 0),
...
// func greedyDecode(model: TransformerModel, input: MotionLangBatch, maxLength: Int, startSymbol: Int32) -> Tensor<Int32> {
//     let memory = model.encode(input: input)
//     var ys = Tensor(repeating: startSymbol, shape: [1,1])
//     // ys = Tensor(copying: ys, to: device)
//     for _ in 0..<maxLength {
//         let decoderInput = MotionLangBatch(tokenIds: input.tokenIds,
//                                      targetTokenIds: ys,
//                                      mask: input.mask,
